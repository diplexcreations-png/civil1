...
import { 
  ...
  Calculator, HardHat, Sliders, Activity, Hammer, CheckCircle, AlertTriangle
} from 'lucide-react';
...
export default function LandingPage({ onSelectCalculator, onLaunchDashboard }: LandingPageProps) {
  ...
  const [simSpan, setSimSpan] = useState<number>(6); // meters
  const [simLoad, setSimLoad] = useState<number>(25); // kN
  const [simRebarSize, setSimRebarSize] = useState<number>(16); // mm rebar diameter
  const [simRebarCount, setSimRebarCount] = useState<number>(4);

  // Live simulator formulas (simplified beam calculations for rendering)
  const simResults = useMemo(() => {
    const spanMm = simSpan * 1000;
    const loadN = simLoad * 1000;
    
    // Moment M = P*L / 4 for simple point load at midspan
    const maxMomentK_Nm = (simLoad * simSpan) / 4;
    
    // Area of Steel Ast
    const rebarArea = (Math.PI * Math.pow(simRebarSize, 2)) / 4;
    const totalAst = rebarArea * simRebarCount; // mm2

    // Inertia estimation (assuming 300x500mm beam)
    const b = 300;
    const h = 500;
    const baseInertia = (b * Math.pow(h, 3)) / 12; // mm4
    
    // Cracked inertia approximation based on rebar area
    const modularRatio = 8;
    const crackedInertia = baseInertia * (0.2 + (totalAst * modularRatio) / (b * h));

    // Elastic Modulus of concrete (~25 GPa)
    const Ec = 25000; // MPa (N/mm2)
    
    // Deflection Delta = P * L^3 / (48 * E * I)
    const maxDeflectionMm = (loadN * Math.pow(spanMm, 3)) / (48 * Ec * crackedInertia);
    
    // Allowable deflection limit L/240
    const allowDeflectionMm = spanMm / 240;
    const isSafe = maxDeflectionMm <= allowDeflectionMm;
    const deflectionRatio = maxDeflectionMm / allowDeflectionMm;

    return {
      moment: maxMomentK_Nm,
      ast: totalAst,
      deflection: maxDeflectionMm,
      allowDeflection: allowDeflectionMm,
      isSafe,
      deflectionRatio
    };
  }, [simSpan, simLoad, simRebarSize, simRebarCount]);
  ...
  return (
    <div className="space-y-16 pb-20">
      ...
      {/* NEW INTERACTIVE BLUEPRINT SIMULATOR SECTION */}
      <section className="max-w-7xl mx-auto px-4">
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/85 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden">
          
          <div className="absolute top-0 right-0 w-80 h-80 bg-blue-500/5 dark:bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-emerald-500/5 dark:bg-emerald-500/5 rounded-full blur-3xl pointer-events-none"></div>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-5 space-y-6 text-left">
              <div className="space-y-2">
                <div className="inline-flex items-center space-x-1.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-450 px-2.5 py-1 rounded-lg text-[9px] font-mono uppercase font-bold tracking-wider border border-emerald-500/20">
                  <Activity className="w-3.5 h-3.5" />
                  <span>Real-time Sandbox</span>
                </div>
                <h3 className="text-xl md:text-2xl font-extrabold font-sans text-slate-900 dark:text-white tracking-tight">
                  Interactive Stress & Deflection Tester
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-sans">
                  Apply virtual forces to a simulated concrete beam. Tweak dimensions, load, and internal rebars to analyze moments and structural limits dynamically.
                </p>
              </div>

              <div className="space-y-4 p-4 bg-slate-50/80 dark:bg-slate-950/60 border border-slate-200/60 dark:border-slate-850 rounded-2xl">
                {/* Span Slider */}
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[11px] font-mono">
                    <span className="text-slate-500 dark:text-slate-450 uppercase font-semibold">Beam Span length</span>
                    <span className="text-blue-500 dark:text-[#0A84FF] font-bold">{simSpan} meters</span>
                  </div>
                  <input 
                    type="range" 
                    min="3" 
                    ...
                    step="0.5"
                    value={simSpan} 
                    onChange={(e) => setSimSpan(parseFloat(e.target.value))}
                    className="w-full accent-[#0A84FF] cursor-pointer h-1.5 bg-slate-200 dark:bg-slate-850 rounded-lg"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[11px] font-mono">
                    <span className="text-slate-500 dark:text-slate-450 uppercase font-semibold">Central Point Load</span>
                    <span className="text-emerald-500 dark:text-emerald-400 font-bold">{simLoad} kN</span>
                  </div>
                  <input 
                    type="range" 
                    min="5" 
                    ...
                    className="w-full accent-emerald-500 cursor-pointer h-1.5 bg-slate-200 dark:bg-slate-850 rounded-lg"
                  />
                </div>

                {/* Rebar Sizes Selector */}
                <div className="grid grid-cols-2 gap-3 pt-1">
                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-wider block">REBAR DIAMETER</label>
                    <select
                      value={simRebarSize}
                      onChange={(e) => setSimRebarSize(parseInt(e.target.value))}
                      className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg py-1.5 px-2.5 text-xs font-mono text-slate-700 dark:text-slate-300 outline-none focus:border-blue-500"
                    >
                      <option value="12">12 mm (#4 size)</option>
                      <option value="16">16 mm (#5 size)</option>
                      <option value="20">20 mm (#6 size)</option>
                      <option value="25">25 mm (#8 size)</option>
                    </select>
                  </div>

                  <div className="space-y-1">
                    <label className="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-wider block">REBAR QUANTITY</label>
                    <select
                      value={simRebarCount}
                      onChange={(e) => setSimRebarCount(parseInt(e.target.value))}
                      className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg py-1.5 px-2.5 text-xs font-mono text-slate-700 dark:text-slate-300 outline-none focus:border-blue-500"
                    >
            ...
            {/* Right side: visual live graph rendering */}
            <div className="lg:col-span-7 flex flex-col justify-center items-center">
              
              {/* Dynamic SVG Beam Bending Graphic */}
              <div className="w-full bg-slate-50 dark:bg-slate-950/80 border border-slate-200/80 dark:border-slate-850 rounded-2xl p-5 relative overflow-hidden flex flex-col items-center">
                
                {/* Simulated Point Load Down Arrow */}
                <div className="absolute top-1 text-center font-mono text-[9px] text-slate-400">
                  APPLIED FORCE
                </div>
                
                {/* SVG Canvas */}
                <svg viewBox="0 0 500 240" className="w-full h-44 drop-shadow-sm select-none">
                  {/* Grid Lines */}
                  <line x1="30" y1="120" x2="470" y2="120" stroke="#cbd5e1" strokeDasharray="3,3" strokeWidth="0.5" className="dark:stroke-slate-800" />
                  <line x1="250" y1="30" x2="250" y2="210" stroke="#cbd5e1" strokeDasharray="3,3" strokeWidth="0.5" className="dark:stroke-slate-800" />

                  {/* Supports (Left & Right pinned endpoints) */}
                  <polygon points="35,120 20,150 50,150" fill="#94a3b8" />
                  <polygon points="465,120 450,150 480,150" fill="#94a3b8" />
                  <line x1="15" y1="150" x2="55" y2="150" stroke="#475569" strokeWidth="2" />
                  <line x1="445" y1="150" x2="485" y2="150" stroke="#475569" strokeWidth="2" />

                  {/* Load Arrow at Center */}
                  <g transform={`translate(250, ${30 + Math.min(25, simLoad / 1.5)})`}>
                    <line x1="0" y1="0" x2="0" y2="40" stroke="#ef4444" strokeWidth="2.5" markerEnd="url(#arrow)" />
                    <text x="8" y="18" fill="#ef4444" fontSize="10" fontWeight="bold" fontFamily="monospace">{simLoad} kN</text>
                    <defs>
                  ...
                  {/* Bending Beam Shape Path */}
                  {/* Midpoint calculated vertical deflection for render */}
                  {/* max calculated deflection in physics represents delta. Let's scale up for visual dramatic flex (e.g. up to 25px max) */}
                  {(() => {
                    const flexScale = Math.min(22, simResults.deflection * 1.8);
                    const controlX = 250;
                    const controlY = 120 + flexScale;
                    const beamPath = `M 35,120 Q ${controlX},${controlY} 465,120`;
                    
                    // Thicker Concrete beam bounding area
                    const topPath = `M 35,100 Q ${controlX},${100 + flexScale} 465,100`;
                    const bottomPath = `M 465,125 Q ${controlX},${125 + flexScale} 35,125`;
                    const renderFullConcretePath = `M 35,100 Q ${controlX},${100 + flexScale} 465,100 L 465,125 Q ${controlX},${125 + flexScale} 35,125 Z`;

                    // Internal Steel Rebars (drawn inside concrete)
                    const steelPath = `M 38,118 Q ${controlX},${118 + flexScale} 462,118`;

                    return (
                      <>
                        {/* Shaded Concrete Beam Block */}
                        <path 
                          d={renderFullConcretePath} 
                        ...
                        {/* Internal steel bar indicator */}
                        <path 
                          d={steelPath} 
                          stroke="#ef4444" 
                          strokeWidth={simRebarSize >= 20 ? "3" : "1.8"} 
                          fill="none" 
                          strokeDasharray="1, 0.5" 
                        />

                        {/* Shear cracks at extreme load under tension */}
                        {simResults.deflectionRatio > 0.7 && (
                          <g stroke="#ef4444" strokeWidth="1.2" opacity={Math.min(1, simResults.deflectionRatio - 0.5)}>
                            <line x1={controlX - 10} y1={controlY} x2={controlX - 15} y2={controlY - 12} />
                            <line x1={controlX} y1={controlY + 2} x2={controlX} y2={controlY - 14} />
                            <line x1={controlX + 10} y1={controlY} x2={controlX + 15} y2={controlY - 12} />
                          </g>
                        )}
                        ...
                        <defs>
                          <pattern id="concretePattern" width="10" height="10" patternUnits="userSpaceOnUse">
                            <rect width="10" height="10" fill="#cbd5e1" className="dark:fill-[#1e293b]" opacity="0.9"/>
                            <circle cx="2" cy="2" r="0.5" fill="#475569" />
                            <circle cx="7" cy="5" r="0.8" fill="#475569" />
                            <path d="M 0 10 L 10 0" stroke="#94a3b8" strokeWidth="0.2" className="dark:stroke-slate-800" />
                          </pattern>
                        </defs>

                        {/* Value Labels */}
                        <text x="35" y="85" fontSize="8" fill="#94a3b8" fontFamily="monospace">SUPPORT L (PIN)</text>
                        <text x="385" y="85" fontSize="8" fill="#94a3b8" fontFamily="monospace">SUPPORT R (ROLLER)</text>
                        
                        {/* Midpoint flex delta arrow */}
                        <g transform={`translate(${controlX}, ${controlY + 8})`}>
                          <line x1="0" y1="0" x2="0" y2="25" stroke="#3b82f6" strokeWidth="1" strokeDasharray="2,2" />
                          <polygon points="0,25 -3,20 3,20" fill="#3b82f6" />
                          <text x="6" y="20" fill="#3b82f6" fontSize="9" fontWeight="bold" fontFamily="monospace">Deflection (Δ)</text>
                        </g>
                      </>
                    );
                  })()}
                ...
                <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-3 border-t border-slate-200 dark:border-slate-850 pt-4 mt-1">
                  
                  <div className="space-y-0.5 text-left pl-2">
                    <span className="text-[8px] font-mono text-slate-400 block uppercase font-bold">Max Moment</span>
                    <span className="text-[11px] font-bold font-mono text-slate-850 dark:text-slate-100">
                      {simResults.moment.toFixed(1)} kN·m
                    </span>
                  </div>

                  <div className="space-y-0.5 text-left">
                    <span className="text-[8px] font-mono text-slate-400 block uppercase font-bold">Steel Area Ast</span>
                    <span className="text-[11px] font-bold font-mono text-slate-850 dark:text-slate-100">
                      {simResults.ast.toFixed(0)} mm²
                    </span>
                  </div>

                  <div className="space-y-0.5 text-left">
                    <span className="text-[8px] font-mono text-slate-400 block uppercase font-bold">Deflection Δ</span>
                    <span className="text-[11px] font-bold font-mono text-slate-850 dark:text-slate-100">
                      {simResults.deflection.toFixed(3)} mm
                    </span>
                  </div>

                  <div className="space-y-0.5 text-left">
                    <span className="text-[8px] font-mono text-slate-400 block uppercase font-bold">Limit Compliance</span>
                    <div className="flex items-center space-x-1">
                      {simResults.isSafe ? (
                        <span className="inline-flex items-center text-[9px] font-bold font-mono text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-md uppercase">
                          <CheckCircle className="w-2.5 h-2.5 mr-0.5" /> OK
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-[9px] font-bold font-mono text-red-500 bg-red-500/10 px-1.5 py-0.5 rounded-md uppercase">
                          <AlertTriangle className="w-2.5 h-2.5 mr-0.5" /> Fails L/240
                        </span>
                      )}
                    </div>
                  </div>
